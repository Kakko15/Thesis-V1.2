...
- **Runtime caveat closed (2026-07-28):** a bounded development smoke (3 fully synthetic queries, `--allow-unvalidated`, output marked `formal_result: false`) ran the complete Objective 2 pipeline end to end — deployed guest RAG path, Ragas 0.4.3 Answer Correctness on both pathways, Faithfulness and Context Precision on retrieved contexts (n=3, none skipped), and the Section 3.2.5 statistics (Shapiro-Wilk → paired t-test). The smoke surfaced and fixed two harness defects that would otherwise have appeared during PI-10: ragas' native google provider builds a synchronous client that can never serve the async `ascore()` API (evaluator now uses Gemini's OpenAI-compatible endpoint with `AsyncOpenAI`, the workaround ragas' own source recommends), and scipy's numpy-bool `normal` flag broke JSON serialization. Sanitized evidence: `rag-thesis-backend/evaluation/results/comparison_20260728_140718.json` (context hashes only, no manuscript text). The formal run still requires the locked corpus and faculty-validated dataset (PI-08/PI-09/PI-10).
