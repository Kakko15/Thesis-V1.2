...
/**
 * Theme/palette/motion/effects/contrast controls. Shared by the Settings →
 * Appearance section and the lightweight appearance dialog on public pages,
 * so guests and signed-in users get the identical control set. The native
 * <select> dropdowns are safe inside dialogs because Modal swallows the
 * popup-dismiss click that would otherwise hit the backdrop.
 */
export function AppearanceControls() {
  const {
    theme, palette, motion, effects, contrast, updatePreference,
  } = usePreferences()
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        <div className="block">
          ...
          <Select value={motion} onChange={(event) => updatePreference('motion', event.target.value)} aria-label="Motion">
        ...
        <div className="block">
          ...
          <Select value={effects} onChange={(event) => updatePreference('effects', event.target.value)} aria-label="Visual effects">
      ...
      <section>
        ...
        <Select value={contrast} onChange={(event) => updatePreference('contrast', event.target.value)} aria-label="Contrast">
