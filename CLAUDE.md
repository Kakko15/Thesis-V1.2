...
- Model names (`gemini-3.8-flash` for chat, extract and verdict; `models/gemini-embedding-001` for embeddings) and every pinned dependency version. `tests/test_release_fingerprint.py` pins the first two as literals, so a model change fails there first. They are printed in the paper's Tables 1-4, which `paper/build_corrections.py` generated before `paper/` was removed — so nothing in the tree now checks a model or version change against those tables; restore the script from git history if you need to. The model names are also in the README. The Dockerfile asserts Python is exactly 3.14.7 and CI installs the same; keep the workflow, README, and paper tables in step when it moves.
