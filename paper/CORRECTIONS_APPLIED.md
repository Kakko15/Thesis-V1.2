...
- **P10-1 Section 1.3 (Dataset Volume Limit)** - the paragraph promised fifty documents and
  ...
  reserved fifty, and that the reduced coverage is a limitation of the evaluation.
- **P10-2 Section 3.2.1 (sampling)** - "will utilize purposive sampling in selecting fifty
  (50) undergraduate thesis documents" is replaced by the census description and the list
  of unrepresented categories. The needle for this edit is the paragraph as the 2026-08-25
  catalog-relabelling pass leaves it, not the 2026-08-09 original, because Pass 10 is
  applied last.
- **P10-3 Section 3.2.1 (Golden Dataset)** - the instrument keeps all forty queries and now
  declares a coverage stratum for each: 3 negative controls, 5 queries scoped by their own
  wording to an unreleased track, and 32 left `undetermined` for the panel to set when it
  drafts the ground truth. Deleting the queries the corpus cannot answer would have quietly
  narrowed the catalog-wide scope the methodology declares; leaving them unmarked would have
  let a correct "no evidence in the corpus" response be scored as an accuracy failure.
  **Answer Correctness** is the metric reported per stratum as well as pooled; Faithfulness
  and Context Precision remain split by response kind, which is what the harness computes.

  The first version of this stratum assignment derived it from each query's category and was
  corrected in review the same day, before any ground truth was drafted. Category records
...
`by_corpus_coverage`) and the 2026-09-07 section of `evaluation/iso25010_evidence.md`.

**`paper_CORRECTED.pdf` is not regenerated by this pass.** It is exported from Word, so it
still shows the pre-Pass-10 text; re-export it from `paper_CORRECTED.docx` before circulating
the PDF.

## Pass 12 - 3.2.5 statistical treatment, 2026-09-13

Section 3.2.1 already defines the four corpus-coverage strata and commits to reporting every
stratum separately as well as pooled. Section 3.2.5 still described a single pooled test, so
the statistical-treatment section did not state the treatment the evaluation actually applies.
Two append-only edits, both landing in the second paragraph of 3.2.5, which is untouched
2026-08-09 original text -- so `PROSE12` is order-independent, unlike Passes 10 and 11.

- **P12-1 interval and effect size per branch** - 3.2.5 promised "a 95% confidence interval
  and a standardized effect size" without saying what the interval is when the rank test is
  chosen. `statistical_treatment` computes the interval at `run_comparison.py:797`, outside
  the parametric/non-parametric branch, so a two-sided Student's t interval accompanies a
  Wilcoxon result too, and both effect sizes are always emitted. The paper now says so.
- **P12-2 stratified treatment and exclusions** - states four things the section omitted:
  the sequence runs per stratum as well as pooled (`:987` pooled, `:1005` per stratum); it
  runs on Answer Correctness alone, since the metric loop at `:966` is the single-element
  tuple `('answer_correctness',)` while Faithfulness and Context Precision go through
  `summarize_rag_diagnostics` (`:714`), which takes no stratum and runs no test; a test is
  withheld for **two** reasons, fewer than three pairs (`:772`) *and* identical paired
  differences (`:774`), with the stratum reported descriptively in either case; and a query
  the provider never processed is excluded from the paired test rather than scored as a
  failure, while a no-evidence notice or guard refusal is not an outage of that kind and is
  scored as returned.

  The pass also fixes which comparison carries the accuracy claim. No family-wise or FDR
  correction exists in the harness -- `significant_at_0.05` (`:806`) is the sole significance
  decision -- so the present-stratum comparison is **fixed in advance** as the accuracy claim
  and the pooled comparison and remaining strata are reported in full but treated as
  secondary. This is a pre-specification, made before the results are written up, and it is
  what keeps a pooled-versus-present divergence reportable rather than a post-hoc choice.

The section stays in the proposal's future tense and carries no measured value: the paper
ends at Chapter 3 and 3.2.5 states treatment, not findings. The only digits added are 0.05,
95%, "three" and the cross-reference to 3.2.1.

Repository ground truth for this pass: `evaluation/run_comparison.py`
(`statistical_treatment` at `:761-812`, the metric loop at `:966`, the per-stratum call at
`:1005`, `summarize_rag_diagnostics` at `:714`) and the 2026-09-13 section of
`evaluation/iso25010_evidence.md`.

**`paper_CORRECTED.pdf` is not regenerated by this pass.** It is exported from Word, so it
still shows the pre-Pass-12 text; re-export it from `paper_CORRECTED.docx` before circulating
the PDF.
