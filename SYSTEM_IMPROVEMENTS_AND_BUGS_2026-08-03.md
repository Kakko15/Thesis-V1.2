...
| **§2.6 Newly discovered (N1–N16)** | **16** | 0 | 0 | Found by audits run *during* remediation, not present in the original report. N15 and N16 are migration-consistency defects surfaced by preparing a disposable project for the load test |
...
| Gate | Before | After |
|---|---|---|
| PyTest | 430 passed / 90.87% | **643 passed, 3 skipped / 91.53%** 📈 (645 passed, 1 skipped when `ALLOW_DISPOSABLE_SUPABASE_TESTS=1` enables the two live disposable-project checks; CI has no `.env`, so 643/3 is the reproducible figure) |
