...
- **Live notice classification** — `ChatResponse.kind` (`answer` | `notice`) is stamped with the same classifier that stores `chat_messages.kind`, so a capacity apology or refusal is distinguishable from a research answer in the live UI, not only after a reload.
...
- **Ingestion worker:** deploy the same image as a separate process with command `python -m workers.ingestion_worker`. Never run production uploads with only the API process. `INGESTION_CONCURRENCY` (default 1) lets one worker process run that many queued jobs at once so a batch upload drains in parallel; leave it at 1 for evaluation runs, and size it to the container's memory because each in-flight job holds a manuscript plus its OCR renders. Batch uploads (`POST /upload/batch`, up to `MAX_BATCH_FILES`, default 20) arrive as one multipart request, so an ingress in front of the API needs a body limit of at least `MAX_BATCH_FILES x MAX_UPLOAD_MB`.
